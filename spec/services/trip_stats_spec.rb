...
RSpec.describe TripStats do
  ...
  context '#weekly_stats', vcr: 'address_cords' do
    ...
    let!(:first_day_of_current_week_10) do
      Trip.create(start_address: 'Pulawska 5', destination_address: 'Lazurowa 40', date: Date.current.beginning_of_week, price: 10)
    end

    let!(:first_day_of_current_week_20) do
      Trip.create(start_address: 'Pulawska 15', destination_address: 'Twarda 40', date: Date.current.beginning_of_week, price: 20)
    end

    let!(:last_day_of_current_week) do
      Trip.create(start_address: 'Pulawska 5', destination_address: 'Lazurowa 40', date: Date.current.end_of_week, price: 10)
    end

    it 'return correct data' do
      weekly_stats = TripStats.weekly_stats

      expect(weekly_stats[:total_distance]).to eq('43.21km')
